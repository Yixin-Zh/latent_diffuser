...
@hydra.main(config_path=".", config_name="dd.yaml", )
def pipeline(args):
    ...
    fix_mask = torch.zeros((args.diffuser_horizon, args.latent_size))
    fix_mask[0] = 1.
    loss_weight = torch.ones((args.diffuser_horizon, args.latent_size))
    ...
    diffuser.load(os.path.join(args.model_path, 'diffusion_ckpt_latest.pt'))
    diffuser.eval()
    ...
    for i in range(1): 
        ...
        while t < 200:
            ...
            img = torch.from_numpy(obs['agentview_image']).to(args.device)
            prior = torch.zeros((1, 8, 256)).to(args.device)
            
            latent_obs = agent.model['encoder'](img.unsqueeze(0)) # (1, 256)
            print(latent_obs[0][:20 ])
            gen_img = agent.model['decoder'](latent_obs) # (1, 3, 84, 84)
            gen_img = 255*gen_img[0]
            
            gen_video.append(gen_img.cpu().detach().numpy().transpose(1, 2, 0).astype(np.uint8))
            prior[:, 0] = latent_obs

            traj, _ = diffuser.sample(
                    prior, solver=args.solver,
                    n_samples=1, sample_steps=args.sampling_steps, use_ema=args.use_ema,
                    temperature=args.temperature)
            

            act = agent.model['inv'](torch.cat([latent_obs, traj[:,1]], dim=-1))
            
            act = act.clamp(-1, 1)
           
            # Denormalize prediction
            action_pred = dataset.normalizers['action'].unnormalize(act.cpu().detach().numpy())
