...
def pipeline(args):
    ...
    for batch in loop_dataloader(dataloader):
        ...
        for i in range(8):
            print(latent_obs[0, i, :10])
        #print(latent_obs[0, 0, :10])
        # latent obs chunk
        latent_obs_chunk = latent_obs.reshape(-1, latent_obs.shape[-1])
        latent_obs_list.append(latent_obs_chunk)
    ...
    print(np.max(latent_obs))
    print(np.min(latent_obs))
    latent_obs = latent_obs.reshape(-1, latent_obs.shape[-1])
    print(latent_obs.shape)
   
    plot_tsne(latent_obs, 'Image Column Space', 'img_tsne.png')
