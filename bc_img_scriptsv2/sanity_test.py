...
def pipeline(args):
    ...
    dataloader = DataLoader(dataset, 1, shuffle=True, num_workers=8, pin_memory=True)

    # ---------------------- Create Agent ----------------------
    agent = Agent(latent_dim=256, action_dim=7, device=args.device)
    model_path = '/home/yixin/Desktop/model10.22/v2'
    agent.load(os.path.join(model_path, 'model_latest'))
    agent.eval()
    # ---------------------- Training ----------------------
    step = 0
    max_steps = 200000
    ...
    for batch in loop_dataloader(dataloader):
        ...
        latent_obs, latent_next_obs = agent.test(obs, next_obs, act, step)
        print("#"*10)
        print(latent_obs[0][:20])
        print(latent_next_obs[0][:20])
