...
class BCObsAgent(BCAgent):
    def __init__(self, obs_dims, act_dims, device, n_layers=3, hidden_dim=256, lr=3e-4):
        ...
        self.obs_enc = functions.build_mlp(self.robot_obs_dim, self.lat_obs_dim, n_layers, hidden_dim,
        ...
        self.obs_dec = functions.build_mlp(self.lat_obs_dim, self.robot_obs_dim, n_layers, hidden_dim,
        ...
        self.inv_dyn = functions.build_mlp(self.lat_obs_dim*2, self.act_dim-1, 
        ...
        self.fwd_dyn = functions.build_mlp(self.lat_obs_dim+self.act_dim-1, self.lat_obs_dim, 
            n_layers, hidden_dim, activation='relu', output_activation='identity', batch_norm=self.batch_norm).to(device)
...
class BCObsActAgent:
    def __init__(self, obs_dims, act_dims, device, n_layers=3, hidden_dim=256, lr=3e-4):
        ...
        self.act_enc = functions.build_mlp(self.robot_obs_dim+self.act_dim-1, self.lat_act_dim, n_layers, 
        ...
        self.act_dec = functions.build_mlp(self.robot_obs_dim+self.lat_act_dim, self.act_dim-1, n_layers,
            hidden_dim, activation='relu', output_activation='tanh', batch_norm=self.batch_norm).to(device)
    ...
    def test_latent(self, obs, deterministic=False):
        with torch.no_grad():
            robot_obs, obj_obs = obs[:, :self.robot_obs_dim], obs[:, self.robot_obs_dim:]
            lat_obs = self.obs_enc(robot_obs)
            lat_act = self.actor(torch.cat([lat_obs, obj_obs], dim=-1))
    ...
    def update(self, obs, act, next_obs, step):
       
        log1 = self.update_dyn_cons(obs, act, next_obs, step)
